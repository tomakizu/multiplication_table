                ...
                <div class="row align-self-center">
                    <?php while ($record = $result->fetch_assoc()) { ?>
                        <div class="col align-self-center border">
                            <p><?= $record['name'] ?><?= $language_string[$_SESSION['lang']]['statistic_2'] ?></p>
                            <table class="table table-bordered">
                                ...
                                for ($i = 2; $i <= 10; $i++) {
                                    $statement = 'SELECT';
                                    $statement .= '( SELECT COUNT(*) FROM quiz_details INNER JOIN quiz_result ON quiz_details.quiz_id = quiz_result.id INNER JOIN question ON quiz_details.question_id = question.id WHERE user_id=? AND number1=? AND marks=1 ) AS correct_count, ';
                                    $statement .= '( SELECT COUNT(*) FROM quiz_details INNER JOIN quiz_result ON quiz_details.quiz_id = quiz_result.id INNER JOIN question ON quiz_details.question_id = question.id WHERE user_id=? AND number1=?) AS total_count';
